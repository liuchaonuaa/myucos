...
void Widget::readData()
{
    ...
    if (usart_temp == "OK")
    {
        ui->loadTextEdit->insertPlainText(tr("程序下载结束\n"));
        usart_temp.clear();
    }
...
void Widget::transmitDataFun(int cnt)   // 发送文件
{
    ...
    //  0x5a 0xb4  index_L index_H data_len_L  data_len_H  data(no more than 1K) CRC
    QByteArray binTransmit;
    binTransmit.resize(10);
    ...
    if (temp < 1024)
    {
        ...
        binTransmit.insert(6, binByte, 1024);

        crc.crc32(&binTransmit, binTransmit.size() - 4);
        unsigned long crc_val = crc.getCrc32();

        binTransmit[1024 + 6] = crc_val;
        binTransmit[1024 + 7] = crc_val >> 8;
        binTransmit[1024 + 8] = crc_val >> 16;
        binTransmit[1024 + 9] = crc_val >> 24;
...
void Widget::on_fileButton_clicked()        //打开文件
{
    ...
    if (fileLocation != NULL)
    {
        ...
        QFile *binFile = new QFile(fileLocation);
        if (!binFile->open(QIODevice::ReadOnly)) //重新打开文件
            emit binFileOpenFailed();
        else
        {
            ui->loadButton->setEnabled(true);
...
void Widget::on_programButton_clicked()
{
    QByteArray binTransmit;
